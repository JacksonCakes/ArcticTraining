...
dataset_name = "twinkle-ai/tw-reasoning-instruct-50k"
total_num_of_scripts = args.total_num_of_scripts
for i in range(total_num_of_scripts):
    output_dir = f"{data_save_folder_name}/tw-reasoning-instruct-50k"
    json_save_path = f"{output_dir}/{i}_{total_num_of_scripts}.jsonl"
    script = f"""
python speculator/data_generation/vllm_gen_full_data.py --model={model_name} --hf_dataset {dataset_name} --tensor_parallel={vllm_tensor_parallel} --tokenizer={tokenizer_name} --cur_split={i} --output_dataset_path={json_save_path} --total_split={total_num_of_scripts}
    """
